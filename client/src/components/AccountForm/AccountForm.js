			...
			<Form
				onSubmit={async values => {
					...
					try {
						...
						this.state.formToggle
							...
							: this.setState({ error: 'Please fill all fields with valid information' });
						throw e;
					}
				}}
				render={({ handleSubmit, pristine, invalid, submitting, form }) => (
					<form onSubmit={handleSubmit} className={classes.accountForm}>
						...
						<Typography className={classes.errorMessage}>{this.state.error ? this.state.error : ''}</Typography>
					</form>
				)}
			/>
		);
